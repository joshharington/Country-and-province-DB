			...
			success: function(res) {
				...
				for(var i = 0; i < allCountries.length; i++) {
					...
					$.ajax({
						url: 'process.php',
						type: 'post',
						data: {'section' : 'country', 'continentid' : continentid, 'country' : allCountries[i] },
						async: false,
						success: function(result) {
							countryid = result;
							// loop through towns
							$.each(city_states, function(k, v) {
								if(allCountries[i] == k) {
									var allTowns = v.split('|');
									for(var j = 0; j < allTowns.length; j++) {
										towns[j] = allTowns[j];
										// insert town / province
										$.ajax({
											url: 'process.php',
											type: 'post',
											data: {'section' : 'town', 'countryid' : countryid, 'town' : allTowns[j] },
											async: false,
											success: function(finalRes) {
												if(finalRes == "success") {
													console.log('town inserted');												
												}											
											},
											error: function(finalRes) {
...
// 		for(var i = 0; i < allCountries.length; i++) {
// 			// get country
// 			country[i] = allCountries[i];
// 			// insert country
// 			// - get Country ID
// 			$.ajax({
// 				url: 'process.php?section=country',
// 				type: 'post',
// 				data: 'country='+allCountries[i]+'&&continentid="+continentid,
// 				success: function(res) {
// 					countryid = res;
// 				},
// 				error: function(res) {
...
// 						data: 'country='+allCountries[i]+'&&continentid="+continentid,
// 						success: function(res) {
// 							console.log(allCountries[i] + ' inserted');
// 							countryid = res;
// 						},
// 						error: function(res) {
// 							console.log(res);
